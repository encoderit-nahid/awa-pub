<form  id="youtube-video-upload-form" method="POST" action="{{ route('add-project') }}" enctype='multipart/form-data'>
    @csrf
    ...
    @endif
      ...
      <div class="form-group row">
        ...
        <div class="col-md-6">
            <input id="youtube" type="file" class="form-control{{ $errors->has('youtube') ? ' is-invalid' : '' }}" name="youtube" value="{{ old('youtube') }}" required accept="video/mp4,video/x-m4v,video/*"
                   max-duration="300">
	...
	<div class="form-group row">
        <label for="extra" class="col-md-4 col-form-label text-md-right">{{ __('Information zur Musiklizenz*') }}</label>

        <div class="col-md-6">
            <textarea id="extra" rows="10" class="form-control{{ $errors->has('extra') ? ' is-invalid' : '' }}" name="extra" value="{{ old('extra') }}" required></textarea>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="checkbox" checked="checked" name="check" id="check" value="{{old('check')}}" required><label for="check"><span></span><p>Ich habe das Recht das Video und die Musik zu verwenden!</p></label>
            @if ($errors->has('check'))
                <span class="invalid-feedback">
...
<link rel="stylesheet" href="{{ asset('preloader/src/css/preloader.css') }}" />
<script src="{{ asset('preloader/src/js/jquery.preloader.min.js') }}" async></script>
<script>
    $( window ).on("load", function() {

            var loaderPosition = parseInt($('body').outerHeight())-300;

            $(document).on("submit", "form", function () {
                $('body').preloader({
                    text: 'Uploading . . .'
                });
                $('.preloader-container').css('top', loaderPosition+'px');
            });
    });
</script>
