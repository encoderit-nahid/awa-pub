                    ...
                    <div class="card-body">
                        ...
                        <section class="projects endless-pagination" data-next-page="{{ $projects->nextPageUrl() }}">
                            @foreach($projects as $project)
                                ...
                                @endif
                                @if($project->rating_visible==0)
                                    <button class="btn btn-primary action-show-rating" dataValue="1" id="{{$project->id}}">Show Rating</button>
                                @else
                                    <button class="btn btn-primary action-show-rating" dataValue="0" id="{{$project->id}}">Hide Rating</button>
                                @endif
                                ...
                                <p style=""><b>Kategorie: {{ $project->cat_name }}</b></p>
                                <p style=""><b>Projektname: {{ $project->projektname }}</b></p>
                                <p style=""><b>Project-ID: {{ $project->id }}</b></p>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on('click', '.action-accept', function () {
                $.ajax({
                    url: "{{ url('/project-accept-admin') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
