<form method="POST" action="{{ route('add-project') }}">
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            ...
            Erlaubte Wörter: {{ $cats->words }} - Wörter total : <span id="display_count_beschreibung">0</span>
            Wörter!
    ...
    <div class="form-group row">
        <label for="extra" class="col-md-4 col-form-label text-md-right">{{ __('Weitere Informationen*') }}</label>

        <div class="col-md-6">
            <textarea id="extra" rows="10" class="form-control{{ $errors->has('extra') ? ' is-invalid' : '' }}"
                name="extra" value="{{ old('extra') }}" required></textarea>
            Erlaubte Wörter: {{ $cats->words }} - Wörter total : <span id="display_count_extra">0</span> Wörter!
            @if ($errors->has('extra'))
                <span class="invalid-feedback">
    ...
    <div class="form-group row">
        <label for="testimonial"
            class="col-md-4 col-form-label text-md-right">{{ __('Referenzen mit Kontaktdaten*') }}</label>

        <div class="col-md-6">
            <textarea id="testimonial" rows="10" class="form-control{{ $errors->has('testimonial') ? ' is-invalid' : '' }}"
                name="testimonial" value="{{ old('testimonial') }}" required></textarea>
            Erlaubte Wörter: <span class="testimonial-word-limit">{{-- $cats->words --}} 600</span> - Wörter total :
            <span id="display_count_referenz">0</span> Wörter!
...
<script>
    $(document).ready(function() {
        $("#testimonial").on('keyup', function() {
            var max = $('.testimonial-word-limit').text(); // {{ $cats->words }};
            var words = this.value.match(/\S+/g).length;
        ...
        $("#extra").on('keyup', function() {
            ...
            if (words > max) {
                ...
                $('#display_count_extra').text(words);
                $('#word_left').text(max - words);
            }
        });
    });
