...
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                <form action="{{ url('/project-show-1st-round') }}">
                                    @csrf
                        ...
                        <section class="projects endless-pagination" data-next-page="{{ $projects->nextPageUrl() }}">
                            @foreach ($projects as $project)
                                ...
                                {{-- @dd($project); --}}
                                <button class="btn btn-primary action-accept-first-round" id="{{ $project->id }}">2.
                                    Runde</button>
                                <button class="btn btn-primary action-reject-first-round" id="{{ $project->id }}">Leider
                                    nein</button>
                                ...
                                <p style=""><b>Kategorie: {{ $project->cat_name }}
                                        ...
                                        {{-- @if ($project->rating_visible)
                                            <p style=""><b>Sternenbewertung anzeigen: Yes</b></p>
                                        @else
                                            <p style=""><b>Sternenbewertung anzeigen: No</b></p>
                                        @endif --}}
                                        <br>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on("click", ".action-free", function() {
                ...
                $.ajax({
                    url: "{{ url('/project-free') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("click", ".action-show-rating", function() {
                var dataValue = $(this).attr('dataValue');
                $(this).attr("disabled", "disabled");
                $.ajax({
                    url: "{{ url('/project-show-rating') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("click", ".action-service", function() {
                ...
                $(this).attr("disabled", "disabled");
                $.ajax({
                    url: "{{ url('/project-service') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("click", ".action-special", function() {
                ...
                $(this).attr("disabled", "disabled");
                $.ajax({
                    url: "{{ url('/project-special') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on('click', '.action-accept-first-round', function() {
                $.ajax({
                    url: "{{ url('/project-accept-1st-round') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on('click', '.action-reject-first-round', function() {
                $.ajax({
                    url: "{{ url('/project-reject-1st-round') }}".replace("http://", "http://"),
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("change", "#category", function() {
                var url = "{{ url('/project-show-1st-round') }}/" + $(this).val();
            ...
            //     $.ajax({
            //         url: "{{ url('/project-delete-admin') }}".replace("http://", "http://"),
            //         type: 'POST',
            //         headers: {
            //             'X-CSRF-TOKEN': token
            //         },
            ...
            //     $.ajax({
            //         url: "{{ url('/project-jury-admin') }}".replace("http://", "http://"),
            //         type: 'POST',
            //         headers: {
            //             'X-CSRF-TOKEN': token
            //         },
            ...
            //     $.ajax({
            //         url: "{{ url('/project-inv-admin') }}".replace("http://", "http://"),
            //         type: 'POST',
            //         headers: {
            //             'X-CSRF-TOKEN': token
            //         },
    ...
    <script>
        ...
        function del(imageName, md5) {
            var token = $('input[name="ajax_token"]').val();
            $.ajax({
                url: "{{ url('/show-delete') }}".replace("http://", "http://"),
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': token
                },
