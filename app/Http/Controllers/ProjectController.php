...
//        }
        ...
        $projects = Project::with(['images', 'firstRoundEvaluation' => function ($query) use ($userId) {
            $query->where('jury_id', $userId)->whereNull('status');
        }])
            ->whereHas('firstRoundEvaluation', function ($query) use ($userId) {
                $query->where('jury_id', $userId)->whereNull('status');
            })
